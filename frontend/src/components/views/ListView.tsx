...
import {
  Clock, User, Tag, CheckCircle2, Trash2, Edit2,
...
export const ListView: React.FC<ListViewProps> = ({
  ...
  return (
    <>
      {/* ── Desktop table (md+) ── */}
      <div className="hidden md:block bg-[#111118] border border-[#2a2a38] rounded-2xl overflow-hidden">
        {/* Table header */}
                  ...
                  {task.subtasks.length > 0 && (
                    <div className="flex items-center gap-2 mt-1">
                      <div className="h-1 flex-1 max-w-[80px] bg-[#2a2a38] rounded-full overflow-hidden">
                        <div className={`h-full rounded-full ${progress === 100 ? 'bg-green-500' : 'bg-[#4169e1]'}`} style={{ width: `${progress}%` }} />
                      </div>
                ...
                <div>
                  {task.dueDate ? (
                    <span className={`flex items-center gap-1 text-xs ${overdue ? 'text-red-400' : 'text-gray-400'}`}>
                      <Clock size={12} />{formatDueDate(task.dueDate)}
                    </span>
                ...
                <div>
                  {task.assignee ? (
                    <span className="flex items-center gap-1 text-xs text-gray-400">
                      <User size={12} /><span className="truncate max-w-[80px]">{task.assignee}</span>
                    </span>
                  ) : <span className="text-xs text-gray-700">—</span>}
                </div>
                {/* Actions */}
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity" onClick={(e) => e.stopPropagation()}>
                  <button onClick={() => onView(task)} className="p-1.5 rounded-lg text-gray-500 hover:text-blue-400 hover:bg-blue-400/10 transition-colors" title="View"><ChevronRight size={15} /></button>
                  <button onClick={() => onEdit(task)} className="p-1.5 rounded-lg text-gray-500 hover:text-yellow-400 hover:bg-yellow-400/10 transition-colors" title="Edit"><Edit2 size={14} /></button>
                  <button onClick={() => onDelete(task.id)} className="p-1.5 rounded-lg text-gray-500 hover:text-red-400 hover:bg-red-400/10 transition-colors" title="Delete"><Trash2 size={14} /></button>
                </div>
              </div>
            );
          })}
        ...
        {tasks.map((task) => {
          ...
          return (
            <div
              key={task.id}
              onClick={() => onView(task)}
              className={`bg-[#111118] border rounded-2xl p-4 cursor-pointer transition-all duration-150 active:scale-[0.99] animate-fade-in
            ...
            >
              ...
              <div className="flex items-start justify-between gap-2 mb-2">
                ...
                <div className="flex items-center gap-1 flex-shrink-0" onClick={(e) => e.stopPropagation()}>
                  <button onClick={() => onEdit(task)} className="p-1.5 rounded-lg text-gray-600 hover:text-yellow-400 hover:bg-yellow-400/10 transition-colors"><Edit2 size={14} /></button>
                  <button onClick={() => onDelete(task.id)} className="p-1.5 rounded-lg text-gray-600 hover:text-red-400 hover:bg-red-400/10 transition-colors"><Trash2 size={14} /></button>
                </div>
              </div>

              {/* Title */}
              <h3 className={`text-sm font-semibold mb-1.5 leading-snug ${task.status === 'done' ? 'line-through text-gray-600' : 'text-white'}`}>
                {task.title}
              </h3>
              ...
              {task.subtasks.length > 0 && (
                <div className="mb-2">
                  ...
                  <span className="text-xs text-gray-600 mt-0.5 block">{task.subtasks.filter(s => s.completed).length}/{task.subtasks.length} subtasks</span>
                </div>
              )}
              ...
              <div className="flex items-center justify-between mt-2 pt-2 border-t border-[#1e1e2e]">
                <div className="flex items-center gap-3">
                  ...
                  {task.assignee && (
                    <span className="flex items-center gap-1 text-xs text-gray-500">
                      <User size={11} />{task.assignee.split(' ')[0]}
                    </span>
                  )}
                </div>
                {/* Status selector */}
                <div onClick={(e) => e.stopPropagation()}>
                  <select
                    value={task.status}
                    onChange={(e) => onStatusChange(task.id, e.target.value as Status)}
                    className={`text-xs font-medium px-2 py-1 rounded-lg border cursor-pointer bg-transparent focus:outline-none ${status.bg} ${status.color} ${status.border}`}
                  >
                    <option value="todo" className="bg-[#111118] text-gray-300">To Do</option>
                    <option value="in-progress" className="bg-[#111118] text-gray-300">In Progress</option>
                    <option value="review" className="bg-[#111118] text-gray-300">Review</option>
