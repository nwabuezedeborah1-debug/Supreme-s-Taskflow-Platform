...
export const TaskCard: React.FC<TaskCardProps> = ({
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      ...
      if (statusMenuOpen && statusRef.current && !statusRef.current.contains(e.target as Node)) setStatusMenuOpen(false);
    };
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2 mb-3">
        <div className="flex items-center gap-1.5 flex-wrap min-w-0">
          <span className={`w-2 h-2 rounded-full flex-shrink-0 mt-0.5 ${priority.dot}`} />
        ...
        <div ref={menuRef} className="relative flex-shrink-0" onClick={(e) => e.stopPropagation()}>
          <button
            ...
            className="p-1.5 rounded-lg text-gray-600 hover:text-gray-300 hover:bg-[#2a2a38] transition-colors
          ...
          {menuOpen && (
            <div className="absolute right-0 top-9 bg-[#1a1a24] border border-[#2a2a38] rounded-xl shadow-2xl z-30 w-40 py-1 animate-scale-in">
              <MenuBtn icon={<ChevronRight size={14} />} onClick={() => { onView(task); setMenuOpen(false); }}>View Details</MenuBtn>
              <MenuBtn icon={<Edit2 size={14} />}        onClick={() => { onEdit(task); setMenuOpen(false); }}>Edit Task</MenuBtn>
              <hr className="border-[#2a2a38] my-1" />
              <MenuBtn icon={<Trash2 size={14} />} danger onClick={() => { onDelete(task.id); setMenuOpen(false); }}>Delete</MenuBtn>
            </div>
          )}
        </div>
      </div>
      ...
      {task.tags.length > 0 && (
        <div className="flex flex-wrap gap-1 mb-3">
          ...
          {task.tags.length > 3 && (
            <span className="px-2 py-0.5 bg-[#2a2a38] text-gray-500 text-xs rounded-full">+{task.tags.length - 3}</span>
          )}
        </div>
      )}
      ...
      <div className="flex items-center justify-between mt-3 pt-3 border-t border-[#1e1e2e] gap-2">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <div ref={statusRef} className="relative" onClick={(e) => e.stopPropagation()}>
            ...
            {statusMenuOpen && (
              <div className="absolute right-0 bottom-9 bg-[#1a1a24] border border-[#2a2a38] rounded-xl shadow-2xl z-30 w-36 py-1 animate-scale-in">
                {STATUS_OPTIONS.map((opt) => (
                  <button
                    key={opt.value}
                    onClick={() => { onStatusChange(task.id, opt.value); setStatusMenuOpen(false); }}
                    className={`w-full flex items-center gap-2 px-3 py-2.5 text-xs transition-colors hover:bg-[#2a2a38]
...
const MenuBtn: React.FC<{
  ...
  <button
    ...
    className={`w-full flex items-center gap-2 px-3 py-2.5 text-sm transition-colors
      ${danger ? 'text-red-400 hover:bg-red-500/10' : 'text-gray-300 hover:bg-[#2a2a38] hover:text-white'}`}
  >
    {icon} {children}
  </button>
);
