...
import { LayoutDashboard, CheckSquare, Calendar, Tag, BarChart2, Settings, Zap } from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({ stats, activeView, onViewChange }) => {
  return (
    <aside className="w-64 flex-shrink-0 bg-[#0d0d15] border-r border-[#1e1e2e] h-screen flex flex-col">
      {/* Logo */}
      <div className="px-6 py-5 border-b border-[#1e1e2e]">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 bg-[#4169e1] rounded-xl flex items-center justify-center shadow-lg shadow-[#4169e1]/20">
            <Zap size={18} className="text-white" />
          </div>
          <div>
            <h1 className="text-base font-bold text-white leading-tight">Supreme's TaskFlow</h1>
        ...
        {stats && (
          <div className="mt-6">
            ...
            <div className="px-3 space-y-2">
              <StatRow label="Total Tasks" value={stats.total} color="text-gray-300" />
              ...
              <StatRow label="Completed" value={stats.done} color="text-green-400" />
              {stats.overdue > 0 && (
